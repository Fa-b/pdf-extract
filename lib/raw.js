 ...
 */
Raw.prototype.process = function(pdf_path, options, callback = undefined) {
  var self = this;
  var text_pages = [];
  var split_output;
  if (!options) {
      ...
      split(pdf_path, function (err, output) {
        ...
        async.forEachSeries(
          ...
          function (pdf_file, cb) {
            convert(pdf_file.file_path, options, function (err, tif_path) {
              ...
              if(callback) callback(null, { path: tif_path, index: index, final: index === zeroBasedNumPages });
              var ocr_flags = [
                '-psm 6'
              ];
