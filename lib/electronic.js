 ...
 */
Electronic.prototype.process = function(pdf_path, options, callback = undefined) {
  var self = this;
  var text_pages = [];
  var split_output;
  ...
  fs.exists(pdf_path, function (exists) {
    ...
    pathhash(pdf_path, function (err, hash) {
      ...
      split(pdf_path, options.pdf_password, function (err, output) {
        if (err) {
          self.emit('error', { error: err, pdf_path: pdf_path});
          return
        }
        ...
        async.forEachSeries(
          ...
          function (file, cb) {
            var zeroBasedNumPages = files.length - 1;
            if(callback) callback(null, { path: file.file_path, index: index, final: index === zeroBasedNumPages });
            index++;
